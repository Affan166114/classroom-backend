...
router.get("/", async (req, res) => {
  try {
    ...
    const offset = (currentPage - 1) * limitPerPage; // it tells about how many records to skip to get to the next page

    const filterConditions = []; // this will store all the conditions for filtering

    // if search query is exists then we are filtering by subject name OR code
    if (search) {
    ...
    // if department query is exists then we are filtering by department name
    if (department) {
      filterConditions.push(ilike(departments.name, `%${department}%`));
    }

    // combining all the filter conditions using AND operator if any condition exists
    const whereClause =
      filterConditions.length > 0 ? and(...filterConditions) : undefined;

    // Count query MUST include the join. it will count all the elements in the page
    ...
    const subjectsList = await db
      .select({
        ...
        }, // getTableColumns returns all the columns of the current table and also allows to add custom columns data from other tables
      })
      .from(subjects)
      .leftJoin(departments, eq(subjects.departmentId, departments.id))
      .where(whereClause)
    ...
    // this is the response that we are sending to the client
    res.status(200).json({
      data: subjectsList,
      pagination: {
        page: currentPage,
